...
# Function to handle potentiometer readings and LED color changes
def handle_potentiometer():
    global last_value
    scaled_value = read_pot()
    if abs(scaled_value - last_value) >= 6:  # Check if the change is significant
        map_value_to_color(scaled_value)  # Change LED color based on value
        last_value = scaled_value  # Update the last known value

# Function to handle network requests
def handle_network_request(scaled_value):
    global last_request_time
    current_time = utime.time()
    if current_time - last_request_time >= 3:  # Check if at least 3 seconds have passed
...
# Main loop
while True:
    handle_potentiometer()  # Handle potentiometer readings and LED updates
    handle_network_request(last_value)  # Handle network requests
    utime.sleep(0.1)  # Short delay to yield control